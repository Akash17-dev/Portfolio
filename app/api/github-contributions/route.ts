...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (contributions.length === 0) {
      return NextResponse.json({ error: "No GitHub contribution data was found." }, { status: 502 });
    }
    ...
    return NextResponse.json({ error: "Unable to fetch GitHub contributions." }, { status: 502 });
  }
}
